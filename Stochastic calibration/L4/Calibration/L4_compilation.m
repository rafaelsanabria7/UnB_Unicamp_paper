...
L4_exp = load('L4_LVDT.csv');

for step = 1:30 % Model 8
output = strcat('L4_',num2str(step),'_FORCE','.tb'); 
% Open model
fid = fopen(output);
 ...
 fclose(fid);
%  
 output = strcat('L4_',num2str(step),'_DISP','.tb');  
 fid = fopen(output);      
        i = 1;
        j = 1;
        k = 1;
...
%  output_3 = strcat('L4_10_',num2str(step),'_DISP','.tb');  
%  fid = fopen(output_3);      
%         i = 1;
%         j = 1;
%         k = 1;
...
u_disp = [4 6 10 14 max(abs(L4_exp(:,5)))];
[new_disp index] = unique(abs(L4_exp(:,5)));

test_force = interp1(new_disp,abs(L4_exp(index,1)),u_disp);
...
plot(-L4_exp(:,5), L4_exp(:,1)+25,'r-.','LineWidth',1.5)
% plot(-L4_exp(:,5), L4_exp(:,1)+25,'k-.')
% plot([0 40], [420 420],'r-')
