...
L3_exp = load('L3_LVDT.csv');

for step = 1:30 % model 12
output = strcat('L3_',num2str(step),'_FORCE','.tb'); 
% Open model
fid = fopen(output);
 ...
 fclose(fid);
%  
 output = strcat('L3_',num2str(step),'_DISP','.tb');  
 fid = fopen(output);      
        i = 1;
        j = 1;
        k = 1;
...
%  output_3 = strcat('L3_10_',num2str(step),'_DISP','.tb');  
%  fid = fopen(output_3);      
%         i = 1;
%         j = 1;
%         k = 1;
...
u_disp = [2 4 6 8 10 12 max(abs(L3_exp(:,5)))];
[new_disp index] = unique(abs(L3_exp(:,5)));

test_force = interp1(new_disp,abs(L3_exp(index,1)),u_disp);
...
plot(-L3_exp(:,5), L3_exp(:,1)+25,'r-.','LineWidth',1.5)
% plot(-L3_exp(:,5), L3_exp(:,1)+25,'k-.')
% plot([0 40], [490 490],'r-')
% 
 fig = gcf;
