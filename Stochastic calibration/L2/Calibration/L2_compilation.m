...
L2_exp = load('L2_LVDT.csv');

for step = 1:30 % model = 3
    
output = strcat('L2_',num2str(step),'_FORCE','.tb'); 
% Open model
fid = fopen(output);
...
output = strcat('L2_',num2str(step),'_DISP','.tb');  
fid = fopen(output);      
        i = 1;
        j = 1;
        k = 1;
...
%  output_3 = strcat('L2_10_',num2str(step),'_DISP','.tb');  
%  fid = fopen(output_3);      
%         i = 1;
%         j = 1;
%         k = 1;
...
u_disp = [2 4 6 max(abs(L2_exp(:,5))) 10 12];
[new_disp index] = unique(abs(L2_exp(:,5)));

test_force = interp1(new_disp,abs(L2_exp(index,1)),u_disp);
...
plot(-L2_exp(:,5), L2_exp(:,1)+25,'r-.','LineWidth',1.5)
% plot(-L2_exp(:,5), L2_exp(:,1),'k-.')
% plot([0 20], [525 525],'r-')
% 
 fig = gcf;
